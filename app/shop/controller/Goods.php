...
class Goods extends BaseShop
{
    ...
    public function lists()
    {
        ...
        if (request()->isAjax()) {
            ...
            //判断会员价插件
            $memberprice_is_exit = addon_is_exit('memberprice');
            ...
            // 判断采集插件
            $goodsgrab_is_exit = addon_is_exit('goodsgrab');
    ...
    public function addGoods()
    {
        if (request()->isAjax()) {
            ...
            $data = [
                ...
                'is_consume_discount' => input('is_consume_discount', 0),//是否参与会员等级折扣
    ...
    public function getGoodsSkuList()
    {
        if (request()->isAjax()) {
            $goods_id = input("goods_id", 0);
            $goods_model = new GoodsModel();
            $res = $goods_model->getGoodsSkuList([ [ 'goods_id', '=', $goods_id ], [ 'site_id', '=', $this->site_id ] ], 'sku_id,sku_name,price,market_price,cost_price,stock,weight,volume,sku_no,sale_num,sku_image,spec_name,goods_id,is_consume_discount,member_price,discount_method,discount_config');
            if (!empty($res['data'])) {
                $res['data'] = $goods_model->getSkuMemberPrice($res['data']);
            }
            return $res;
        }
    }
...
//            $this->forthMenu([ 'member_id' => $member_id ]);
    ...
    public function aftersale()
    {
        ...
        if (request()->isAjax()) {
            ...
            return $this->fetch('goods/aftersale');
        }
    }
}
