...
class Goodssku extends BaseApi
{
    ...
    public function detail()
    {
        ...
        if ($token[ 'code' ] >= 0) {
            ...
            // 是否参与会员等级折扣
            $goods_member_price = $goods_model->getGoodsPrice($sku_id, $this->member_id);
            $goods_member_price = $goods_member_price['data'];
    ...
    public function page()
    {
        ...
        gs.introduction,gs.promotion_type,gs.create_time,gs.goods_state,gs.goods_class,gs.goods_class_name,gs.max_buy,gs.min_buy,g.goods_image,g.site_name,gs.goods_spec_format,gs.is_virtual,gs.recommend_way';
        ...
        if (!empty($attr)) {
            ...
            foreach($attr as $k => $v){
                ...
                if($attr_is_first){
                    ...
                    if(!empty($attr_sku_ids)){
                        $attr_sku_ids = array_merge($attr_sku_ids, $item_attr_sku_ids);
                    }else{
                       continue;
                    }
            ...
            $attr_sku_ids = array_unique($attr_sku_ids);
            $condition[] = [
