<?php /*a:2:{s:75:"/www/wwwroot/niushop1.bucuoya.cn/app/admin/view/upgrade/upgrade_action.html";i:1619855886;s:57:"/www/wwwroot/niushop1.bucuoya.cn/app/admin/view/base.html";i:1619851154;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<script type="text/javascript" src="http://niushop1.bucuoya.cn/app/admin/view/public/js/progressbar.js"></script>
<link rel="stylesheet" type="text/css" href="http://niushop1.bucuoya.cn/app/admin/view/public/css/upgrade/style.css" media="screen">
<link rel="stylesheet" type="text/css" href="http://niushop1.bucuoya.cn/app/admin/view/public/css/upgrade/jquery.easy-pie-chart.css" media="screen">
<style>
	.chart{
...
<script type="text/javascript">
	...
	upgrade_obj.totalActionEnd = function(){
		...
		layer.msg('升级完成', function(index, layero){
			window.location.href = ns.url("admin/upgrade/upgrade");
		})
	};
	...
	upgrade_obj.backupFile = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/backupFile"),
			dataType : 'json',
			success:function(res){
				if(res.code >= 0){
	...
	upgrade_obj.backupSql = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/backupSql"),
			dataType : 'json',
			data : {
	...
	upgrade_obj.downloadFileExec = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/download"),
			dataType : 'json',
			data:{
	...
	upgrade_obj.executeFile = function(){
		...
		if(that.data.files.length > 0){
			$.ajax({
				type:'post',
				url : ns.url("admin/upgrade/executeFile"),
				dataType : 'json',
				data:{},
				success:function(data){
					if(data.code >= 0){
	...
	upgrade_obj.executeSql = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/executeSql"),
			dataType : 'json',
			success:function(data){
				if(data.code >= 0){
					that.total_action.status[method] = 'end';
	...
	upgrade_obj.upgradeStart = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/upgradeStart"),
			dataType : 'json',
			success:function(res){
				if(res.code >= 0){
					that.total_action.status[method] = 'end';
	...
	upgrade_obj.upgradeEnd = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/upgradeEnd"),
			dataType : 'json',
			success:function(res){
				if(res.code >= 0){
					that.total_action.status[method] = 'end';
	...
	upgrade_obj.executeRecovery = function(){
		...
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/executeRecovery"),
			dataType : 'json',
			data : {
	...
	function getUpgradeInfo() {
		$.ajax({
			type:'post',
			url : ns.url("admin/upgrade/upgradeAction"),
			dataType : 'json',
			success:function(res){
				if(res.code >= 0){
