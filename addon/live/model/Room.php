...
class Room extends BaseModel
{
    ...
    public function addRoom($data)
    {
        $liver_data = array (
            ...
            'close_replay' => $data['closeReplay'],
            'close_share' => $data['closeShare'],
            'close_kf' => $data['closeKf']
        );
        ...
        if (!empty($room_list)) {
            foreach ($room_list as $item) {
                ...
                if (!empty($room_info)) {
                    ...
                    //购物直播频道封面图
                    if (is_url($item[ 'feeds_img' ])) {
                        $pull_result = $upload->remotePull($item[ 'feeds_img' ]);
                        $pull_result = $pull_result[ 'data' ];
                        if (isset($pull_result[ 'pic_path' ]) && !empty($pull_result[ 'pic_path' ])) {
                            $data[ 'feeds_img' ] = $pull_result[ 'pic_path' ];
                        }
                    }
    ...
    public function agree($id)
    {
        ...
        if (!empty($info)) {
            ...
            $feeds_img = $upload_model->setPath("common/temp/" . date("Ymd") . '/')->remotePullToLocal($info[ 'feeds_img' ])[ 'data' ][ 'path' ] ?? '';
            $feeds_media_result = $live->addImageMedia($feeds_img);
            if ($feeds_media_result[ 'code' ] < 0)
                return $feeds_media_result;

            $feeds_media_id = $feeds_media_result[ 'data' ][ 'media_id' ] ?? '';
            ...
            $create_data = [
                ...
                'feedsImg' => $feeds_media_id,
                'closeReplay' => $info['close_replay'],
                'closeShare' => $info['close_share'],
                'closeKf' => $info['close_kf']
            ];
