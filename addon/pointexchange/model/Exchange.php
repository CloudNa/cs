...
class Exchange extends BaseModel
{
    ...
    public function addExchange($data)
    {
        ...
        if ($data[ 'type' ] == 1) {
            $exist = model('promotion_exchange')->getInfo([ [ 'type_id', '=', $data[ 'gift_id' ] ], ['type', '=', $data['type']] ], 'id');
            if (!empty($exist)) {
            ...
            $exist = model('promotion_exchange')->getInfo([ [ 'type_id', '=', $data[ 'coupon_type_id' ] ], ['type', '=', $data['type']]  ], 'id');
            if (!empty($exist)) {
    ...
    {
        $info = model("promotion_exchange")->getInfo([ [ 'id', '=', $id ] ], 'id,type,type_name,type_id,name,image,stock,point,market_price,price,delivery_price,balance,state,content,pay_type');
