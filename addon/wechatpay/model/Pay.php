        ...
        switch ($param["trade_type"]){
            case 'JSAPI' ://微信支付 或小程序支付
                if( $this->is_weapp == 0){
                    ...
                    if ($is_matched) $config['orderInfo'] = $this->getOrderInfo($param['out_trade_no'], $openid, $result['prepay_id']);
                    $return = array(
                        "type" => "jsapi",
                        "data" => $config
                    );
...
//        $desc_data["refund_desc"] = $param["refund_reason"];// 商家退款原因 暂时不考虑
    ...
    private function getOrderInfo($out_trade_no, $openid, $prepay_id){
        ...
        model('order')->update(['is_video_number' => 1], [['order_id', '=', $order_info['order_id'] ]]);
        // if (addon_is_exit('shopcomponent', $order_info['site_id'])) (new Weapp($order_info['site_id']))->addOrder($data);
        return $data;
    }
}