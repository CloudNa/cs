...
class OrderExport extends BaseModel
{
     ...
     * @param $condition
     */
    public function orderExport($condition, $condition_desc, $site_id = 0, $app_module = 'shop', $join = [])
    {
        ...
        try {
            ...
            if (dir_mkdir($file_path)) {
                ...
                $order_table = Db::name('supply_order')->where($condition)->alias($alias);
                ...
                $order_table->field($table_field)->chunk(500, function($item_list) use ($fp, $temp_line, $field_key_array) {
                    //写入导出信息
     ...
     * @param $condition
     */
    public function orderGoodsExport($condition, $condition_desc, $site_id = 0, $app_module='shop')
    {
        ...
        try {
            ...
            $order_table = Db::name('supply_order_goods')->where($condition)->alias($alias);
    ...
    public function itemExport($item_list, $field_key, $temp_line, $fp)
    {
        ...
        foreach ($item_list as $k => $item_v) {
            $item_v['province_name'] = model('area')->getInfo(['id' => $item_v['province_id']])['name'];
            $item_v['city_name'] = model('area')->getInfo(['id' => $item_v['city_id']])['name'];
            $item_v['district_name'] = model('area')->getInfo(['id' => $item_v['district_id']])['name'];
