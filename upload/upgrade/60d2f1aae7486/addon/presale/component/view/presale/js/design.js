			...
			presaleListHtml += '<div class="layui-input-block">';
				...
				presaleListHtml += '<ul class="ns-icon">';
					presaleListHtml += '<li v-for="(item, index) in goodsSources" v-bind:k="index" :class="[item.value == data.sources ? \'ns-text-color ns-border-color ns-bg-color-diaphaneity\' : \'\']" @click="data.sources=item.value">';
						presaleListHtml += '<img v-if="item.value == data.sources" :src="item.selectedSrc" />';
						presaleListHtml += '<img v-else :src="item.src" />';
					presaleListHtml += '</li>';
				presaleListHtml += '</ul>';
			...
			goodsSources: [
				{
					text: "默认",
					value: "default",
					src: presaleResourcePath + "/presale/img/goods.png",
					selectedSrc: presaleResourcePath + "/presale/img/goods_1"+url+".png"
				},
				{
					text : "手动选择",
					value : "diy",
					src: presaleResourcePath + "/presale/img/manual.png",
					selectedSrc: presaleResourcePath + "/presale/img/manual_1"+url+".png"
				}
			],
			categoryList: [],
			isLoad: false,
...
Vue.component("change-type", {
	...
	data: function () {
		return {
			data: this.$parent.data,
			changeTypeList: [
				{name: "平移滑动", value: 1, src: presaleResourcePath + "/presale/img/manual.png", selectedSrc: presaleResourcePath + "/presale/img/manual_1.png"},
				{name: "切屏滑动", value: 2, src: presaleResourcePath + "/presale/img/manual.png", selectedSrc: presaleResourcePath + "/presale/img/manual_1.png"},
			]
		};
	},
	created: function () {
