...
//        $cart_ids, $member_id
        ...
        $field = ' sku_id, sku_name, sku_no, 
            ...
            is_free_shipping, shipping_template, goods_class, goods_class_name, goods_id,max_buy,min_buy,goods_name,sku_spec_format';
        ...
        if (!empty($goods_list)) {
            foreach ($goods_list as $k => $v) {
                ...
                $goods = new Goods();
                // 是否参与会员等级折扣
                $price_result = $goods->getGoodsPrice($v['sku_id'], $data['member_id']);
...
//                }
    ...
    public function getOrderCouponList($shop_goods, $data)
    {
        ...
        //TODO  预售兼容
        if(isset($shop_goods['shop_goods_list'])){
            $shop_goods_list = $shop_goods['shop_goods_list'];
        }else{
...
//                    $shop_info = $shop_info_result['data'];
    ...
    public function invoice($shop_goods, $data, $self = null)
    {
        ...
        $invoice_status = $invoice_config['invoice_status'] ?? 0;
...
//            $invoice_content_array = explode(',', $invoice_content);
            ...
            if ($is_invoice) {
                ...
                if(!empty($shop_invoice_item['is_tax_invoice']))
                {
                    $shop_goods['is_tax_invoice'] = $shop_invoice_item['is_tax_invoice'];
                }
                if(!empty($shop_invoice_item['taxpayer_number']))
                {
